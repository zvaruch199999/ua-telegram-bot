...
async def s_street(message: Message, state: FSMContext):
...
async def s_rent(message: Message, state: FSMContext):
...
async def s_dep(message: Message, state: FSMContext):
...
async def s_com(message: Message, state: FSMContext):
...
async def s_park(message: Message, state: FSMContext):
...
async def s_move(message: Message, state: FSMContext):
...
async def s_view(message: Message, state: FSMContext):
...
async def s_broker(message: Message, state: FSMContext):
...
async def finish_offer(message: Message, state: FSMContext):
    ...
    if photos:
        await bot.send_photo(GROUP_CHAT_ID, photos[0], caption=text)
        for p in photos[1:]:
            await bot.send_photo(GROUP_CHAT_ID, p)
    else:
